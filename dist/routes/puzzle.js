...
router.post("/upload", adminAuth, async (req, res) => {
    ...
    try {
       ...
       RETURNING id`, [puzzleDate]);
        const puzzleId = puzzleDay.rows[0].id;
...
router.get("/demo", async (req, res) => {
    try {
        ...
        const fetchDay = async (date, language) => {
         ...
         JOIN puzzles p ON p.id = pc.puzzle_id
         JOIN puzzle_types pt ON pt.id = pc.puzzle_type_id
         WHERE p.puzzle_date = $1 ${whereLang}`, params);
            return rows;
        };
        // Find latest date with any content <= today
        const latestAny = await db.query(`SELECT p.puzzle_date
       FROM puzzles p
       JOIN puzzle_content pc ON pc.puzzle_id = p.id
       WHERE p.puzzle_date <= $1
       ORDER BY p.puzzle_date DESC
       LIMIT 1`, [today]);
        const targetDate = latestAny.rows?.[0]?.puzzle_date || null;
        ...
        if (rows.length) {
            const haveTypes = new Set(rows.map((r) => r.type_name));
            ...
            if (missingTypes.length) {
                ...
                for (const r of anyLangRows) {
                    if (missingTypes.includes(r.type_name))
                        rows.push(r);
                }
            }
        }
...
router.get("/export", async (req, res) => {
    ...
    try {
        ...
        const { rows } = await db.query(`SELECT p.puzzle_date, pt.type_name, pc.language, pc.content
       FROM puzzle_content pc
       JOIN puzzles p ON p.id = pc.puzzle_id
       JOIN puzzle_types pt ON pt.id = pc.puzzle_type_id
       ${where}
       ORDER BY p.puzzle_date, pt.type_name`, params);
        if (!rows.length)
