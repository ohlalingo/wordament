...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        const puzzleRow = await db.query(`SELECT pc.puzzle_id, pc.content
       FROM puzzle_content pc
       WHERE pc.id = $1`, [puzzleContentId]);
        ...
        const existing = await db.query(`SELECT 1
         FROM puzzle_attempts pa
        WHERE pa.user_id = $1 AND pa.puzzle_content_id = $2
        LIMIT 1`, [userId, puzzleContentId]);
