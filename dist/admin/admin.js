...
const ADMIN_LOGO_DATA_URL = "data:image/svg+xml;utf8," +
    encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="420" height="140" viewBox="0 0 420 140" fill="none">
      ...
      <g transform="translate(30 70)" font-family="Inter, Arial, sans-serif" font-size="16" font-weight="700" fill="#d60000">
        ${"CYBERWORDAMENT"
        .split("")
        .map((c, i) => `<rect x="${i * 26}" y="0" width="24" height="24" rx="3" ry="3" fill="none" stroke="#d60000" stroke-width="1.2"/><text x="${i * 26 + 12}" y="16" text-anchor="middle">${c}</text>`)
        .join("")}
      </g>
    </svg>`);
...
export async function buildAdminRouter(dbUrlConfig) {
  ...
  `);
    ...
    await dataSource.query(`CREATE UNIQUE INDEX IF NOT EXISTS ux_puzzle_content_day_type_lang_slot ON puzzle_content (puzzle_id, puzzle_type_id, language, slot);`);
                    ...
                    actions: {
                            ...
                            after: async (response) => {
                                ...
                                for (const record of response.records) {
                                    ...
                                    if (params.puzzleId) {
                                        ...
                                        if (puzzle) {
                                            const d = puzzle.puzzleDate ?? puzzle.puzzle_date;
                                            params.puzzle_date =
                                                typeof d === "string"
                                                    ? d
                                                    : d?.toISOString
                                    ...
                                    if (params.puzzleTypeId) {
                                        ...
                                        if (type) {
                                            params.type_name = type.type_name ?? type.typeName;
                                        }
                                    }
                        ...
                        createPuzzleForm: {
                            ...
                            handler: async (request, _response, context) => {
                                ...
                                const normalizedLang = language === "Japanese"
                                    ...
                                    : language === "English"
                                        ...
                                        : typeof language === "string"
                                            ...
                                            : language;
                                const normalizedExternalId = typeof externalId === "string" && externalId.trim() ? externalId.trim() : null;
                                if (!cleanType) {
                                ...
                                const finalContent = typeof content === "string" ? content : JSON.stringify(stripReservedFields(content));
                        ...
                        list: {
                            after: async (response, _req, _ctx) => {
                                ...
                                for (const record of response.records) {
                                    ...
                                    if (userId) {
                                        const user = await Attempt.getRepository().manager.query(`SELECT name, email, region, language FROM users WHERE id = $1 LIMIT 1`, [userId]);
                                        if (user?.[0]) {
                                    ...
                                    if (puzzleContentId) {
                                        const pc = await Attempt.getRepository().manager.query(`SELECT p.puzzle_date, pc.language
                       FROM puzzle_content pc
                       JOIN puzzles p ON p.id = pc.puzzle_id
                       WHERE pc.id = $1
                       LIMIT 1`, [puzzleContentId]);
                                        if (pc?.[0]) {
        ...
        dashboard: {
            ...
            handler: async (_req, res) => {
                try {
                    ...
                    const [lastAttempt] = await mgr.query(`SELECT pa.created_at, u.name, u.region
             FROM puzzle_attempts pa
             JOIN users u ON u.id = pa.user_id
             ORDER BY pa.created_at DESC NULLS LAST
             LIMIT 1`);
                    const [counts] = await mgr.query(`SELECT
             ...
             JOIN puzzles p ON p.id = pc.puzzle_id`);
                    const [users] = await mgr.query(`SELECT
