  ...
  const admin = new AdminJS({
    ...
    resources: [
      ...
      { resource: PuzzleType, options: { navigation: false } },
      {
        ...
        options: {
          parent: { name: "Puzzles" },
          listProperties: ["external_id", "puzzle_date", "type_name", "language"],
            ...
            list: {
              after: async (response) => {
                ...
                for (const record of response.records) {
                  ...
                  if (params.puzzle_id) {
                    const puzzle = await dataSource.getRepository(Puzzle).findOneBy({ id: params.puzzle_id })
                    if (puzzle) {
                      params.puzzle_date = puzzle.puzzleDate
                    }
                  }

                  if (params.puzzle_type_id) {
                    const type = await dataSource.getRepository(PuzzleType).findOneBy({ id: params.puzzle_type_id })
                    if (type) {
